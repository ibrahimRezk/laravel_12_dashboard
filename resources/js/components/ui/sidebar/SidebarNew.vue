...
/>
  ...
  >
    ...
    <div
      :class="cn(
        'duration-200 fixed inset-y-0 z-10 hidden h-svh w-[--sidebar-width] transition-[left,right,width] ease-linear md:flex bg-sidebar/80 dark:bg-sidebar/10  ',
        side === 'left'
          ? 'left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]'
          : 'right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]',
        // Adjust the padding for floating and inset variants.
        variant === 'floating' || variant === 'inset'
          ? 'p-2 rounded-xl group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)] '
...
<div
  class="w-full h-full rounded-lg absolute top-0 left-0 bg-[url('/assets/img/noise.jpg')] bg-contain bg-center opacity-10  -z-20 "
/>
<div
  class="w-full h-full rounded-lg absolute top-0 left-0 bg-[url('/assets/img/grid.svg')] bg-contain bg-center opacity-30  dark:opacity-10  -z-20 "
/>
        <slot />
      </div>
      </div>
